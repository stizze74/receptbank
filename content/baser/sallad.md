---
namn: Grön sallad med olja & vinäger
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 3
  fett: 14
  protein: 1
  kcal: 153
tid:
  prep_min: 5
  tillagning_min: 0
  total_min: 5
forvantad_glukospaverkan: mycket-lag
taggar:
  - vegetariskt
  - kall
  - snabb
ingredienser:
  - mangd: 100
    enhet: g
    vara: blandsallad
    notering: 'rucola, ekblad, frisé, romaine eller blandning'
  - mangd: 0.5
    enhet: st
    vara: gurka
    notering: skivad
  - mangd: 6
    enhet: st
    vara: körsbärstomater
    notering: delade
  - mangd: 2
    enhet: msk
    vara: olivolja
  - mangd: 1
    enhet: msk
    vara: rödvinsvinäger
  - mangd: 1
    enhet: tsk
    vara: dijonsenap
    notering: emulgerar dressingen
  - mangd: 0.5
    enhet: tsk
    vara: flingsalt
  - mangd: 1
    enhet: krm
    vara: svartpeppar
tillagning:
  - namn: Skölj sallad
    text: Skölj salladen om nödvändigt. Riv eller skär i bitar. Lägg i en skål.
  - namn: Skär gurka och tomater
    text: Skiva gurkan i halvmånar. Dela tomaterna på mitten.
  - namn: Gör dressing
    text: >-
      Vispa ihop olivolja, vinäger, dijonsenap, salt och peppar i en liten skål
      — eller skaka i en burk med lock.
  - namn: Mixa
    text: Häll dressingen över salladen precis innan servering. Vänd om försiktigt.
tips:
  - Dressing alltid SIST — sallad som stått länge i dressing blir slapp.
  - 'Bästa dressingen är 3:1 olja till syra. Justera efter smak.'
  - >-
    Centrifugera salladen torr i salladsslunga om du har — våt sallad späder ut
    dressingen.
  - 'Avokado, nötter eller riven ost höjer mättnaden om salladen är huvudrätt.'
testlogg: []
bild: "sallad.jpg"
bild_alt: "AI-bild: Grön sallad med olja & vinäger"
bild_ai: true
---

Den enklaste basen i banken. Färska bladsallader, en god dressing, klart. Tar 5 minuter, kräver inget från spisen, fungerar som lättare bas till nästan alla proteiner.

## Modulkombinationer som funkar

- **Lunch:** Sallad + räkor + avokado + Lohmanders aioli.
- **Vardag, lättare middag:** Sallad + smörstekt kycklingbröst + citronsmör.
- **Helg, sommar:** Sallad + entrecôte i strimlor + örtsmör.
- **Med fisk:** Sallad + smörstekt lax + citronsmör.

## Bygg din egen sallad

Använd 100 g blandsallad som bas. Tillsätt sedan från följande:

| Tillägg | Mängd för 2 | Höjer kh? |
|---|---|---|
| Gurka | 1/2 st | Nej |
| Tomater (cocktail/körsbär) | 5–6 st | Lite (~2 g pp) |
| Avokado | 1/2 st | Nej (mest fiber) |
| Rädisor | 5–6 st | Nej |
| Färsk paprika | 1/4 st | Lite (~1 g pp) |
| Rödlök | 1 msk | Lite |
| Nötter (mandel, valnöt) | 1 msk | Nej |
| Riven ost | 30 g | Nej |
| Oliver | 8–10 st | Nej |

## Dressing-grunden

3:1 olja till syra (vinäger eller citron). En tesked dijon emulgerar och ger djup. Salt och peppar.
