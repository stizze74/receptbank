---
namn: LCHF-hamburgerbullar (mandelmjölsbas)
status: skriven
portioner: 4
narings_per_portion:
  netto_kh: 3
  fett: 18
  protein: 11
  kcal: 240
tid:
  prep_min: 10
  tillagning_min: 20
  total_min: 30
forvantad_glukospaverkan: mycket-lag
taggar:
  - bröd
  - bak
  - snabb
  - burgare
ingredienser:
  - mangd: 1.5
    enhet: dl
    vara: mandelmjöl
  - mangd: 2
    enhet: msk
    vara: kokosmjöl
  - mangd: 1
    enhet: msk
    vara: psyllium husk
  - mangd: 1
    enhet: tsk
    vara: bakpulver
  - mangd: 0.5
    enhet: tsk
    vara: salt
  - mangd: 3
    enhet: st
    vara: ägg
  - mangd: 2
    enhet: msk
    vara: smält smör
  - mangd: 2
    enhet: msk
    vara: 'vatten, ljummet'
  - mangd: 1
    enhet: tsk
    vara: äppelcidervinäger
  - mangd: 1
    enhet: msk
    vara: sesamfrön
    notering: valfritt på toppen
tillagning:
  - namn: Förvärm ugn
    text: 'Sätt ugnen på 175°C, varmluft. Bakplåtspapper på plåt.'
  - namn: Blanda torrt
    text: 'Blanda mandelmjöl, kokosmjöl, psyllium husk, bakpulver och salt.'
  - namn: Blanda vätskan
    text: 'Vispa ägg, smält smör, vatten och vinäger.'
  - namn: Förena
    text: >-
      Vänd samman till en len, fast deg. Låt stå 5 min så psylliumen suger
      vätska.
    timer_min: 5
  - namn: Forma
    text: >-
      Dela degen i 4 lika delar. Forma till runda, lite plattade bullar (~8 cm
      diam). Lägg på plåt med 4 cm mellan. Pensla med ett upvispat ägg om du
      vill (extra glans) och strö sesam ovanpå.
  - namn: Grädda
    text: Grädda 18–20 min tills gyllene och fasta. Färdig när ihl kommer ut torr.
    timer_min: 19
  - namn: Svalna och dela
    text: Låt svalna helt. Dela med brödkniv före användning.
tips:
  - >-
    Funkar som klassisk hamburger-bulle — stadig nog att hålla en saftig
    burgare.
  - Fryser bra hela eller delade.
  - 'Variera: byt sesam mot vallmö eller solrosfrön på toppen.'
  - 'För soft texture: tillsätt 1 msk grädde i vätskan.'
testlogg: []
bild: "lchf-hamburgerbullar.jpg"
bild_alt: "AI-bild: LCHF-hamburgerbullar (mandelmjölsbas)"
bild_ai: true
---


