---
namn: LCHF-kanelbullar
status: skriven
portioner: 10
narings_per_portion:
  netto_kh: 4
  fett: 22
  protein: 8
  kcal: 270
tid:
  prep_min: 25
  tillagning_min: 20
  total_min: 60
forvantad_glukospaverkan: mycket-lag
taggar:
  - brod
  - bullar
  - sot
  - bak
  - kanel
  - fika
ingredienser:
  - mangd: 3
    enhet: dl
    vara: mandelmjöl
  - mangd: 3
    enhet: msk
    vara: kokosmjöl
  - mangd: 2
    enhet: msk
    vara: psyllium husk
  - mangd: 2
    enhet: msk
    vara: erytritol
  - mangd: 2
    enhet: tsk
    vara: bakpulver
  - mangd: 0.5
    enhet: tsk
    vara: salt
  - mangd: 3
    enhet: st
    vara: ägg
  - mangd: 75
    enhet: g
    vara: smält smör
  - mangd: 1
    enhet: dl
    vara: 'vatten, ljummet'
  - mangd: 2
    enhet: msk
    vara: kanel
    notering: till fyllning
  - mangd: 4
    enhet: msk
    vara: erytritol
    notering: till fyllning
  - mangd: 50
    enhet: g
    vara: smör
    notering: 'mjukt, till fyllning'
  - vara: agg till pensling
  - mangd: 1
    enhet: msk
    vara: pärlsocker LCHF (erytritol-grain)
    notering: 'till toppning, valfritt'
tillagning:
  - namn: Förvärm ugn
    text: 'Sätt ugnen på 175°C, varmluft.'
  - namn: Blanda torrt
    text: >-
      Blanda mandelmjöl, kokosmjöl, psyllium husk, erytritol, bakpulver och
      salt.
  - namn: Blanda vätskan
    text: 'Vispa ägg, smält smör och vatten.'
  - namn: Förena
    text: >-
      Vänd samman till en deg. Låt stå 10 min så psylliumen suger vätska och
      degen blir hanterbar.
    timer_min: 10
  - namn: Gör fyllning
    text: 'Blanda mjukt smör, kanel och erytritol till en pasta.'
  - namn: Kavla och fyll
    text: >-
      Kavla ut degen mellan två bakplåtar till en rektangel (ca 30x20 cm). Bred
      ut fyllningen jämnt.
  - namn: Rulla och skär
    text: >-
      Rulla ihop från långsidan. Skär i 10 bitar med vass kniv. Lägg på plåt med
      bakplats, snittsidan uppat.
  - namn: Pensla och toppa
    text: Pensla med upvispat ägg. Strö över pärlsocker om du har.
  - namn: Grädda
    text: Grädda 18–20 min tills gyllene och spröda på ytan.
    timer_min: 19
tips:
  - >-
    LCHF-deg är mer fragil än vanlig vetebrödsdeg — bakplåtspapper hjälper vid
    utkavling.
  - >-
    Pärlsocker LCHF kan vara svårt att hitta — hoppa över eller använd
    grovkornig erytritol.
  - Fryser bra — töa och värm lätt innan servering.
  - >-
    Variera fyllning: kardemumma + erytritol, eller kakao + erytritol för
    choklad-version.
  - >-
    Inte exakt sasom vanlig kanelbulle texturmaslig — mer som scone än bulle.
    Men smaken är där.
testlogg: []
bild: "lchf-kanelbullar.jpg"
bild_alt: "AI-bild: LCHF-kanelbullar"
bild_ai: true
---


