---
namn: Chokladmousse med mörk choklad
status: skriven
portioner: 4
narings_per_portion:
  netto_kh: 4
  fett: 30
  protein: 3
  kcal: 320
tid:
  prep_min: 10
  tillagning_min: 0
  total_min: 10
forvantad_glukospaverkan: mycket-lag
taggar:
  - efterratt
  - kall
  - festligt
  - choklad
ingredienser:
  - mangd: 2
    enhet: dl
    vara: vispgrädde
  - mangd: 100
    enhet: g
    vara: mörk choklad
    notering: minst 70%
  - mangd: 1
    enhet: msk
    vara: 'kakao, osötad'
  - mangd: 1
    enhet: msk
    vara: erytritol/Sukrin (eller efter smak)
  - mangd: 0.5
    enhet: tsk
    vara: vaniljpulver
  - vara: nypa salt
tillagning:
  - namn: Smälta choklad
    text: >-
      Smälta chokladen i vattenbad eller mikro på låg effekt. Rör tills len. Låt
      svalna 5 min.
  - namn: Vispa grädde
    text: >-
      Vispa grädden till mjuka toppar (inte hela vägen). Tillsätt erytritol,
      vanilj, salt och kakao.
  - namn: Vänd ihop
    text: >-
      Vänd försiktigt ner den ljumna chokladen i grädden med slickepott. Inte
      rör ut luften.
  - namn: Kyl
    text: Forma upp i 4 glas. Ställ i kyl minst 1 timme.
    timer_min: 60
tips:
  - Toppa med några hallon eller en klick lattvispad grädde.
  - >-
    70% mörk choklad har ca 30 g kh per 100 g — 25 g per portion ger ~7 g kh
    från chokladen ensam, men fettet bär upp så nettokh per portion håller sig
    låg.
  - Funkar att göra dagen innan — blir bättre.
testlogg: []
---


