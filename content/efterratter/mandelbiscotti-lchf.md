---
namn: Mandelbiscotti LCHF
status: skriven
portioner: 12
narings_per_portion:
  netto_kh: 3
  fett: 14
  protein: 7
  kcal: 180
tid:
  prep_min: 15
  tillagning_min: 45
  total_min: 60
forvantad_glukospaverkan: mycket-lag
taggar:
  - efterratt
  - bak
  - kaffe
  - italiensk
ingredienser:
  - mangd: 200
    enhet: g
    vara: mandelmjöl
  - mangd: 5
    enhet: msk
    vara: erytritol/Sukrin
  - mangd: 1
    enhet: tsk
    vara: bakpulver
  - mangd: 1
    enhet: tsk
    vara: vaniljpulver
  - vara: nypa salt
  - mangd: 2
    enhet: st
    vara: ägg
  - mangd: 2
    enhet: msk
    vara: smält smör
  - mangd: 75
    enhet: g
    vara: mandlar
    notering: hela
  - mangd: 1
    enhet: tsk
    vara: mandelarom
tillagning:
  - namn: Förvärm ugn
    text: 'Sätt ugnen på 175°C, varmluft.'
  - namn: Blanda torrt
    text: 'Blanda mandelmjöl, erytritol, bakpulver, vanilj och salt.'
  - namn: Tillsätt vätska
    text: >-
      Rör ner ägg, smält smör och mandelarom. Vinda i mandlarna. Forma till en
      log på plat med bakplats (~25 cm lång).
  - namn: Första bakning
    text: Baka 25 min tills gyllene och fast.
    timer_min: 25
  - namn: Skär och baka igen
    text: >-
      Låt svalna 10 min. Skär logen i sneda skivor (~1,5 cm). Lägg tillbaka på
      plat, skärsidan ner. Baka 15 min till tills spröda.
    timer_min: 15
tips:
  - >-
    Klassisk italiensk dippeskeks — perfekt till espresso eller vin santo (men
    senare är inte LCHF).
  - Håller 2 veckor i lufttat burk.
testlogg: []
---


