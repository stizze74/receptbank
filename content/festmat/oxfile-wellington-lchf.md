---
namn: Oxfile Wellington LCHF (utan deg)
status: skriven
portioner: 4
narings_per_portion:
  netto_kh: 4
  fett: 32
  protein: 48
  kcal: 480
tid:
  prep_min: 30
  tillagning_min: 25
  total_min: 65
jessica_tagg: byt-protein
forvantad_glukospaverkan: lag
taggar:
  - festmat
  - notkott
  - festligt
  - helg
  - fransk
ingredienser:
  - mangd: 600
    enhet: g
    vara: oxfile
    notering: tunnskären
  - mangd: 200
    enhet: g
    vara: blomkål
    notering: för duxellesersättning
  - mangd: 1
    enhet: st
    vara: schalottenlök
  - mangd: 2
    enhet: klyftor
    vara: vitlök
  - mangd: 50
    enhet: g
    vara: smör
  - mangd: 2
    enhet: msk
    vara: dijonsenap
  - mangd: 100
    enhet: g
    vara: rokt skinka
    notering: Parmaskinka
  - mangd: 2
    enhet: msk
    vara: färsk timjan
  - vara: 'salt, svartpeppar'
  - mangd: 1
    enhet: st
    vara: 'ägg, till pensling'
tillagning:
  - namn: Förber filet
    text: >-
      Salta, peppra. Stek filet i smör på mycket hög värme 1 min per sida tills
      färgat. Pensla med dijonsenap. Låt vila.
    timer_min: 5
  - namn: Gör duxelles-ersättning
    text: >-
      Mixa blomkål fint i matberedare. Fräs med finhackad lök, vitlök och timjan
      i smör tills helt torrt och färgat. Smaka av med salt och peppar.
    timer_min: 12
  - namn: Inveckla
    text: >-
      Bred ut Parmaskinka på plastfolie. Bred blomkålsblandningen över. Lägg
      filet ovanpå och rulla in stramt med skinkan.
  - namn: Ugnsstek
    text: >-
      Lägg på plåt. Pensla med upvispat ägg. Ugn 200°C i 12–15 min till en
      innertemperatur på 56°C för medium-rare.
    timer_min: 14
  - namn: Vila
    text: Låt vila 8 min innan skäring. Skär i tjocka skivor.
    timer_min: 8
tips:
  - LCHF-version utan smordeg — skinkan håller allt samman.
  - Servera med béarnaisesa s (Lohmanders eller hemmagjord) och blomkålspuré.
  - 'Festmat-nivå: passar perfekt till en specialkväll eller jul.'
testlogg: []
bild: "oxfile-wellington-lchf.jpg"
bild_alt: "AI-bild: Oxfile Wellington LCHF (utan deg)"
bild_ai: true
---


