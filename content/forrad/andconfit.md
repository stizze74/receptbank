---
namn: Andconfit (anklar i fett)
status: skriven
portioner: 4
narings_per_portion:
  netto_kh: 0.5
  fett: 32
  protein: 26
  kcal: 380
tid:
  prep_min: 20
  tillagning_min: 180
  total_min: 720
forvantad_glukospaverkan: mycket-lag
taggar:
  - forrad
  - anka
  - festmat
  - fransk
ingredienser:
  - mangd: 4
    enhet: st
    vara: anklar
  - mangd: 100
    enhet: g
    vara: havssalt
  - mangd: 4
    enhet: klyftor
    vara: vitlök
  - mangd: 4
    enhet: kvist
    vara: timjan
  - mangd: 2
    enhet: st
    vara: lagerblad
  - mangd: 500
    enhet: g
    vara: ankfett
    notering: smält eller olivolja som ersättning
tillagning:
  - namn: Salta
    text: >-
      Gnid anklarna med salt, vitlök och timjan. Lägg i kyl 8–12 h (eller över
      natt).
    timer_min: 480
  - namn: Skölj
    text: Skölj av saltet och torka anklarna ordentligt med papper.
  - namn: Confit
    text: >-
      Smält ankfett (eller olivolja) i ugnsförema kastrull. Lägg i anklarna helt
      täckta. Tillsätt lagerblad. Confit på 100°C ugn i 3 timmar tills mycket
      möra.
    timer_min: 180
  - namn: Förvara
    text: >-
      Låt svalna i fettet. För över i burk med anklarna helt täckta av fett.
      Förvara i kyl.
  - namn: Servera
    text: >-
      Stek anklarna i het stekpanna 5 min per sida från burken tills skinnet är
      krispigt. Klassiskt mot lite hagtorn-saltsa s eller en bitter sallad.
tips:
  - >-
    Håller 3 månader i kyl täckt av fett — ett av världens äldsta
    konserveringssatt.
  - Det smaks aturade ankfettet är en guldgruva — använd som stekfett.
testlogg: []
bild: "andconfit.jpg"
bild_alt: "AI-bild: Andconfit (anklar i fett)"
bild_ai: true
---


