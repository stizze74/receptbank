---
namn: Vitlöksconfit
status: skriven
portioner: 12
narings_per_portion:
  netto_kh: 1.5
  fett: 12
  protein: 0.5
  kcal: 110
tid:
  prep_min: 10
  tillagning_min: 60
  total_min: 70
forvantad_glukospaverkan: mycket-lag
taggar:
  - forrad
  - ugn
  - klassiker
  - vegetariskt
ingredienser:
  - mangd: 200
    enhet: g
    vara: vitlöksklyftor
    notering: skalade
  - mangd: 2.5
    enhet: dl
    vara: olivolja
  - mangd: 2
    enhet: kvist
    vara: rosmarin
  - mangd: 2
    enhet: kvist
    vara: timjan
  - mangd: 2
    enhet: st
    vara: lagerblad
  - mangd: 0.5
    enhet: tsk
    vara: svartpepparkorn
  - vara: salt
tillagning:
  - namn: Förvärm ugn
    text: 'Sätt ugnen på 120°C, undervarme/varmluft.'
  - namn: Förber
    text: >-
      Lägg skalade vitlöksklyftor i en eldfast form (ca 20x15 cm). Tillsätt
      rosmarin, timjan, lagerblad, pepparkorn och en nypa salt.
  - namn: Täck med olja
    text: Häll över olivolja så vitlöken täcks helt — viktigt!
  - namn: Confit
    text: >-
      Lagg in i ugnen. Confit 50–60 min tills vitlöken är mjuk och guld-gul. Den
      ska inte brännas.
    timer_min: 55
  - namn: Svalna
    text: >-
      Låt svalna helt. För över vitlök och olja i en ren glasburk. Vitlöken ska
      täckas helt av olja — då håller den.
tips:
  - 'Håller 2 veckor i kyl, täckt av olja.'
  - >-
    Användningsområden: mosa på bröd som smör-ersättning, vinda i
    såser/dressings, på grillat kött, eller bara mos och blanda med yoghurt för
    dipp.
  - >-
    Den smaksatta oljan är guld värd — använd som vinaigrette-bas eller
    stekfett.
  - 'Funkar lika bra med näst an vilken ört som helst: salvia, oregano, basilika.'
testlogg: []
---


