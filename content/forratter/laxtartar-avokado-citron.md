---
namn: Laxtartar med avokado och citron
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 2
  fett: 22
  protein: 22
  kcal: 290
tid:
  prep_min: 10
  tillagning_min: 0
  total_min: 10
forvantad_glukospaverkan: mycket-lag
taggar:
  - kall
  - snabb
  - festligt
  - fisk
  - forratt
ingredienser:
  - mangd: 200
    enhet: g
    vara: färsk lax av sushi-kvalitet
  - mangd: 1
    enhet: st
    vara: avokado
    notering: mogen
  - mangd: 1
    enhet: msk
    vara: olivolja
  - mangd: 1
    enhet: tsk
    vara: pressad citron
  - mangd: 1
    enhet: msk
    vara: 'färsk dill, hackad'
  - mangd: 1
    enhet: msk
    vara: 'gräslök, hackad'
  - vara: 'flingsalt, svartpeppar'
  - mangd: 1
    enhet: tsk
    vara: löjrom
    notering: valfritt
tillagning:
  - namn: Forbered lax
    text: Skär laxen i 5 mm-tärningar med vass kniv. Håll kall hela tiden.
  - namn: Tärna avokado
    text: Tärna avokadon i samma storlek. Vanda i citron direkt så den inte mortnar.
  - namn: Blanda
    text: >-
      Vänd lax och avokado försiktigt med olivolja, dill, gräslök. Salta,
      peppra.
  - namn: Servera
    text: Forma med en ring på tallrik för snyggt. Toppa ev. med löjrom.
tips:
  - Köp lax av sushi-kvalitet eller frusen lax (innehar parasitfri).
  - Funkar på LCHF-skivbröd som canapé — perfekt förrätt-snack.
  - Persilja eller mynta funkar i stället för dill.
testlogg: []
---


