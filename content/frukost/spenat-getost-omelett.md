---
namn: Spenat- och getost-omelett
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 2
  fett: 32
  protein: 24
  kcal: 420
tid:
  prep_min: 5
  tillagning_min: 8
  total_min: 13
forvantad_glukospaverkan: mycket-lag
taggar:
  - frukost
  - brunch
  - agg
  - vegetariskt-protein
  - snabb
ingredienser:
  - mangd: 4
    enhet: g
    vara: stora ägg
  - mangd: 100
    enhet: g
    vara: färsk spenat
  - mangd: 75
    enhet: g
    vara: färsk getost
    notering: 'mild, smulad'
  - mangd: 30
    enhet: g
    vara: smör
  - mangd: 1
    enhet: msk
    vara: vispgrädde
  - vara: 'salt, svartpeppar'
tillagning:
  - namn: Förber spenat
    text: Smält halva smöret i panna. Wilte spenaten 1 min tills bara sjunkit ihop.
    timer_min: 1
  - namn: Vispa ägg
    text: 'Vispa ägg och grädde. Salta, peppra.'
  - namn: Gör omelett
    text: >-
      Tillsätt resterande smör i pannan. Häll i äggsmeten på medel-låg värme.
      Lyft kanterna lätt och låt flytande del rinna ut. När nastan stelt: lägg
      spenat och getost på ena halvan. Vinda omeletten.
    timer_min: 5
  - namn: Servera
    text: Glid på tallrik. Skär på mitten för två portioner.
tips:
  - >-
    Färsk getost — INTE den starka ("chevre mûr") som Stefan undviker. Den milda
    funkar bra.
  - Variera med tärnade körsbärstomater och basilika.
testlogg: []
---


