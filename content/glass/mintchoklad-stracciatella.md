---
namn: Mintchoklad-stracciatella glass
status: skriven
portioner: 6
narings_per_portion:
  netto_kh: 4
  fett: 30
  protein: 4
  kcal: 320
tid:
  prep_min: 15
  tillagning_min: 15
  total_min: 240
forvantad_glukospaverkan: mycket-lag
taggar:
  - glass
  - choklad
  - mint
  - kall
  - vegetariskt
ingredienser:
  - mangd: 4
    enhet: dl
    vara: vispgrädde
  - mangd: 1.5
    enhet: dl
    vara: högfett mjölk
  - mangd: 4
    enhet: st
    vara: äggulor
  - mangd: 3
    enhet: msk
    vara: erytritol pudd
  - mangd: 1
    enhet: tsk
    vara: vaniljpulver
  - mangd: 2
    enhet: tsk
    vara: pepparmyntsextrakt
  - vara: nypa salt
  - mangd: 75
    enhet: g
    vara: mörk choklad
    notering: 'minst 70%, hackad'
tillagning:
  - namn: Gör krame-bas
    text: >-
      Vispa äggulor med erytritol. Värm grädde, mjölk och salt. Rör in i
      äggulorna stegvis. Tjocka i kastrull till 82°C.
    timer_min: 10
  - namn: Smaksätt
    text: >-
      Rör ner vanilj och pepparmyntsextrakt. Sila och svala helt i kyl, helst
      över natt.
  - namn: Frys
    text: >-
      Glassmaskin: kör enligt anvisning, tillsätt hackad choklad sista minuten.
      Manuellt: frys 30 min, vispa, tillsätt choklad, frys 1–2 h till.
  - namn: Servera
    text: Ta ut 5 min innan servering för krameringd textur.
tips:
  - >-
    Klassisk stracciatella — chokladen sprider sig som små skärvor genom
    glassen.
  - Pepparmyntsextrakt finns i alla väl-sortidae mataffe rer.
testlogg: []
---


