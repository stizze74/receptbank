---
namn: Smörkola-glass LCHF
status: skriven
portioner: 6
narings_per_portion:
  netto_kh: 3
  fett: 32
  protein: 4
  kcal: 330
tid:
  prep_min: 15
  tillagning_min: 20
  total_min: 240
forvantad_glukospaverkan: mycket-lag
taggar:
  - glass
  - karamell
  - kall
  - vegetariskt
ingredienser:
  - mangd: 4
    enhet: dl
    vara: vispgrädde
  - mangd: 1
    enhet: dl
    vara: högfett mjölk
  - mangd: 4
    enhet: st
    vara: äggulor
  - mangd: 4
    enhet: msk
    vara: erytritol-honung (Sukrin)
  - mangd: 2
    enhet: msk
    vara: vaniljpulver
  - mangd: 1
    enhet: tsk
    vara: flingsalt
  - mangd: 30
    enhet: g
    vara: smör
tillagning:
  - namn: Gör karamell
    text: >-
      Värm smör och erytritol-honung i kastrull tills lätt karamellfargat
      (medel-låg värme, varsamt). Tar ca 5 min.
    timer_min: 5
  - namn: Tillsätt vätska
    text: Tillsätt grädde och mjölk varsamt (kokar lätt upp). Rör till len.
  - namn: Tjocka
    text: >-
      Vispa äggulor lätt. Tillsätt lätt i karamellkramen. Värm till 82°C, rör
      konstant. Tillsätt vanilj och salt.
    timer_min: 10
  - namn: Sval och frys
    text: >-
      Sila. Sval i kyl över natt. Frys enligt glassmaskin eller manuellt (vispa
      var 30 min).
tips:
  - >-
    Salt-karamell-balans är kritisk — använd flingsalt och var inte snal med
    det.
  - 'Topping-idé: extra erytritol-karamellsa s eller krossade pekannötter.'
testlogg: []
bild: "smorkola-glass-lchf.jpg"
bild_alt: "AI-bild: Smörkola-glass LCHF"
bild_ai: true
---


