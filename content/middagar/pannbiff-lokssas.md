---
namn: Pannbiff med lökssås
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 5
  fett: 38
  protein: 38
  kcal: 540
tid:
  prep_min: 10
  tillagning_min: 25
  total_min: 35
jessica_tagg: byt-protein
forvantad_glukospaverkan: lag
taggar:
  - middag
  - svensk
  - husmans
  - klassiker
  - vardag
ingredienser:
  - mangd: 500
    enhet: g
    vara: nötfärs (högre fetthalt)
  - mangd: 1
    enhet: st
    vara: ägg
  - mangd: 2
    enhet: msk
    vara: vispgrädde
  - mangd: 1
    enhet: tsk
    vara: salt
  - mangd: 0.5
    enhet: tsk
    vara: svartpeppar
  - mangd: 50
    enhet: g
    vara: smör
  - mangd: 2
    enhet: st
    vara: gul lök
    notering: skivad
  - mangd: 2
    enhet: dl
    vara: nötbuljong
  - mangd: 2
    enhet: dl
    vara: vispgrädde
  - mangd: 1
    enhet: msk
    vara: soja
  - mangd: 1
    enhet: tsk
    vara: dijonsenap
tillagning:
  - namn: Forma biffar
    text: >-
      Blanda färs med ägg, grädde, salt och peppar. Forma 4 plata biffar (~1,5
      cm tjocka).
  - namn: Stek lök
    text: >-
      Smält halva smöret. Stek löken på medel-låg värme 8 min tills mjuk och
      latt karamelliserad. Lägg åt sidan.
    timer_min: 8
  - namn: Stek biffar
    text: >-
      Stek biffar i resterande smör på medel-hög värme 3 min per sida. Lägg åt
      sidan.
    timer_min: 6
  - namn: Gör sås
    text: >-
      Avsläck pannan med buljong, koka in 2 min. Tillsätt grädde, soja och
      dijonsenap. Sjud 5 min. Tillsätt löken tillbaka och vinda samman.
    timer_min: 7
  - namn: Servera
    text: >-
      Lägg biffarna på tallrik. Häll över löksåsen. Bra med blomkålsmos eller
      smorstekt vitkal.
tips:
  - >-
    Klassisk lokstark variant — om du har tålamod, låt löken bli riktigt
    karamelliserad (15 min på låg).
  - Funkar lika bra med fläskfars eller blandfars.
testlogg: []
---


