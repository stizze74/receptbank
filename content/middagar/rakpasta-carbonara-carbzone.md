---
namn: Räkpasta carbonara (Carbzone)
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 6
  fett: 38
  protein: 44
  kcal: 580
tid:
  prep_min: 10
  tillagning_min: 18
  total_min: 28
jessica_tagg: delad-fisk
forvantad_glukospaverkan: lag
taggar:
  - middag
  - italiensk
  - skaldjur
  - snabb
  - pasta-version
ingredienser:
  - mangd: 200
    enhet: g
    vara: Carbzone-pasta
    notering: lagkolhydrat-pasta
  - mangd: 300
    enhet: g
    vara: stora räkor
  - mangd: 100
    enhet: g
    vara: rokt fläsksida
    notering: i strimlor
  - mangd: 4
    enhet: st
    vara: äggulor
  - mangd: 75
    enhet: g
    vara: parmesan
    notering: riven
  - mangd: 1
    enhet: dl
    vara: vispgrädde
  - mangd: 2
    enhet: klyftor
    vara: vitlök
  - mangd: 30
    enhet: g
    vara: smör
  - mangd: 1
    enhet: msk
    vara: färsk persilja
  - vara: 'salt, svartpeppar'
tillagning:
  - namn: Koka pasta
    text: >-
      Koka Carbzone-pasta enligt anvisning. Spara 1 dl kokvatten innan du slår
      av.
    timer_min: 10
  - namn: Stek fläsk
    text: 'Stek fläsksida i smör tills krispigt. Tillsätt skivad vitlök, fräs 30 sek.'
    timer_min: 6
  - namn: Stek räkor
    text: 'Tillsätt räkor, stek 2 min tills rosa.'
    timer_min: 2
  - namn: Gör carbonara
    text: 'Vispa äggulor, halva parmesanen och grädde i skål. Salta och peppra lätt.'
  - namn: Förena
    text: >-
      TA AV VÄRMEN på pannan. Häll i den varma pastan och vänd runt. Häll i
      ägg-blandningen och vänd snabbt så äggen tjockna mjuka (inte skirar).
      Tillsätt något av kokvattnet om för torrt.
  - namn: Servera
    text: >-
      Fördela på två tallrikar. Toppa med resterande parmesan och persilja.
      Servera direkt.
tips:
  - 'Pasta-version-tagg — Jessica kan ta vanlig pasta, Stefan tar Carbzone.'
  - >-
    Carbonara-magisken: äggen ska tjocknas av värmen i pasta-pannan men inte
    skirar. Tålamod mört frågan.
testlogg: []
---


