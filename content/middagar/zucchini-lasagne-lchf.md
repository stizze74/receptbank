---
namn: Zucchini-lasagne LCHF
status: skriven
portioner: 4
narings_per_portion:
  netto_kh: 6
  fett: 36
  protein: 38
  kcal: 540
tid:
  prep_min: 25
  tillagning_min: 45
  total_min: 70
forvantad_glukospaverkan: lag
taggar:
  - middag
  - italiensk
  - helg
  - ugn
  - klassiker
ingredienser:
  - mangd: 2
    enhet: st
    vara: stora zucchini
    notering: ca 400 g vardera
  - mangd: 500
    enhet: g
    vara: blandfärs
  - mangd: 1
    enhet: st
    vara: 'gö lök, finhackad'
  - mangd: 3
    enhet: klyftor
    vara: vitlök
  - mangd: 2
    enhet: msk
    vara: tomatpuré
  - mangd: 2
    enhet: dl
    vara: tomater
    notering: krossade
  - mangd: 1
    enhet: tsk
    vara: oregano
  - mangd: 250
    enhet: g
    vara: ricotta-ost
  - mangd: 1
    enhet: st
    vara: ägg
  - mangd: 200
    enhet: g
    vara: mozzarella
    notering: riven
  - mangd: 50
    enhet: g
    vara: parmesan
    notering: riven
  - vara: 'salt, svartpeppar'
tillagning:
  - namn: Förber zucchini
    text: >-
      Skiva zucchini på langden, 0,5 cm tjocka. Salta lätt och låt stå 15 min,
      sen torka med papper för att få ut vätska.
    timer_min: 15
  - namn: Gör köttfärssås
    text: >-
      Fräs lök, vitlök och färs tills färgad. Tillsätt tomatpuré, krossade
      tomater, oregano, salt och peppar. Sjud 15 min.
    timer_min: 18
  - namn: Blanda ricotta
    text: 'Blanda ricotta med ägg, salt och peppar.'
  - namn: Bygg
    text: >-
      Sätt ugnen på 200°C. I eldfast form: lägg lager zucchini, köttfärssås,
      ricotta, mozzarella. Upprepa 2 lager. Toppa med mer mozzarella och
      parmesan.
  - namn: Gratinera
    text: Gratinera 25–30 min tills gyllene och bubblig.
    timer_min: 28
tips:
  - >-
    Salta zucchini och pressa ut vätska — viktigt för att lasagnet inte blir
    blot.
  - 'Bäst dagen efter, smakerna mögnar.'
  - Funkar lika bra med aubergine i stället för zucchini.
testlogg: []
---


