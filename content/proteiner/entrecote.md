---
namn: Stekt entrecôte
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 0
  fett: 57
  protein: 38
  kcal: 670
tid:
  prep_min: 35
  tillagning_min: 12
  total_min: 47
jessica_tagg: byt-protein
forvantad_glukospaverkan: mycket-lag
taggar:
  - notkott
  - helg
ingredienser:
  - mangd: 400
    enhet: g
    vara: entrecôte
    notering: 'gärna 2,5–3 cm tjock skiva'
  - mangd: 30
    enhet: g
    vara: smör
  - mangd: 1
    enhet: msk
    vara: olivolja
  - mangd: 1
    enhet: st
    vara: vitlöksklyfta
    notering: 'krossad, oskalad'
  - mangd: 2
    enhet: kvist
    vara: färsk timjan
    notering: 'eller 0,5 tsk torkad'
  - mangd: 1
    enhet: tsk
    vara: flingsalt
  - mangd: 0.5
    enhet: tsk
    vara: 'svartpeppar, nymalen'
tillagning:
  - namn: Rumstempererat kött
    text: >-
      Ta ut entrecôtet ur kylen 30–60 min innan stekning. Kallt kött rakt från
      kylen blir ojämnt stekt.
    timer_min: 30
  - namn: Förberedelse
    text: >-
      Klappa köttet torrt med hushållspapper. Salta och peppra generöst på båda
      sidor.
  - namn: Het panna
    text: >-
      Värm en gjutjärnspanna eller tjockbottnad stekpanna på hög värme tills
      mycket het. Tillsätt olivolja.
  - namn: Stek sida 1
    text: >-
      Lägg i entrecôtet. Stek utan att röra tills underkanten har en mörkbrun,
      knaprig yta.
    timer_min: 3
  - namn: Vänd
    text: Vänd köttet. Stek på andra sidan med samma höga värme.
    timer_min: 3
  - namn: 'Sänk värmen, arosera'
    text: >-
      Sänk till medelhög värme. Tillsätt smör, vitlök och timjan. Skeda det
      smaksätta smöret över köttet kontinuerligt. Stek till önskad innertemp —
      52–54°C för medium rare.
    timer_min: 2
  - namn: Vila
    text: >-
      Lyft köttet till en skärbräda. Täck löst med folie. Vila — detta är INTE
      valfritt, köttet låser in saften.
    timer_min: 8
  - namn: Skär upp
    text: Skär mot fibrerna i 1 cm tjocka skivor. Servera med pannskyn skedad över.
tips:
  - >-
    Rumstemperatur + vila efter stekning = saftigt kött. Skippa endera och du
    tappar halva poängen.
  - >-
    Termometer i köttet är enda sättet att träffa exakt innertemp.
    Tids-uppskattningar fungerar inte när tjocklek varierar.
  - >-
    Bottnen i pannan efter stekning är perfekt start för en rödvinssås.
    Avglacera med en skvätt vatten eller buljong, koka ihop med smör.
  - Skär ALLTID mot fibrerna — gör köttet mörare i munnen.
testlogg: []
---

Helgmiddagens krona. Entrecôte tål — och kräver — hög värme och kort tid. Två faktorer avgör resultatet: rumstempererat kött innan stekning, och vila efter. Resten är ren teknik.

## Modulkombinationer som funkar

- **Klassiker:** Entrecôte + Lohmanders bea + smörstekta haricots verts.
- **Helg, mer pyssel:** Entrecôte + hollandaise + bakad rotsellerimos + sparris.
- **Lite kontrast:** Entrecôte + örtsmör smält över + grön sallad.
- **Klassisk pepparbiff:** Entrecôte + senap-grädde-sås (med extra peppar) + smörstekt vitkål.

## Jessica-anpassning (byt-protein)

Hon kör eget protein bredvid. Bea, hollandaise och rotsellerimos är delningsbara.

## Innertemperaturer

- **Rare:** 50–52°C
- **Medium rare:** 52–54°C ← rekommenderat
- **Medium:** 56–58°C
- **Medium well:** 60–62°C
- **Well done:** 65°C+ (gör inte detta med entrecôte)

Använd termometer. Tjockleken på köttet avgör hur lång tid det tar — inte recepttiderna.

## Köpval

- 2,5–3 cm tjock skiva är ideal — får fin yta utan att vara genomstekt invändigt.
- För tunn skiva (under 2 cm) blir antingen rå mitt eller överstekt yta.
- Marmorering = smak. Leta efter vita ådringar i köttet.
