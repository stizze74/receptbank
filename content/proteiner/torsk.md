---
namn: Smörstekt torsk
status: skriven
portioner: 2
narings_per_portion:
  netto_kh: 0
  fett: 21
  protein: 45
  kcal: 360
tid:
  prep_min: 8
  tillagning_min: 7
  total_min: 15
jessica_tagg: delad-fisk
forvantad_glukospaverkan: mycket-lag
taggar:
  - fisk
  - snabb
  - vardag
ingredienser:
  - mangd: 500
    enhet: g
    vara: torskfilé
    notering: 'skinnfri, t.ex. ryggbit'
  - mangd: 30
    enhet: g
    vara: smör
  - mangd: 1
    enhet: msk
    vara: olivolja
  - mangd: 1
    enhet: tsk
    vara: flingsalt
  - mangd: 1
    enhet: krm
    vara: 'svartpeppar, nymalen'
  - mangd: 0.5
    enhet: st
    vara: citron
    notering: klyftor till servering
tillagning:
  - namn: Salta i förväg
    text: >-
      Lägg torskfiléerna på ett fat. Salta båda sidor med flingsalt. Låt stå i
      5–10 min — saltet drar ut lite vätska och ger fastare textur.
    timer_min: 8
  - namn: Torka
    text: >-
      Torka filéerna torra med hushållspapper. Detta är kritiskt — våt fisk
      steks inte, den kokas.
  - namn: Värm pannan
    text: >-
      Värm smör och olivolja i stekpanna på medelhög värme tills smöret tystnat
      och fått nötig doft.
  - namn: Stek sida 1
    text: Lägg i torsken. Stek utan att röra tills underkanten är gyllenbrun.
    timer_min: 4
  - namn: Vänd och stek klart
    text: >-
      Vänd försiktigt med stekspade. Skeda smör över. Stek tills fisken flagar
      lätt med gaffel — innertemp 50–55°C.
    timer_min: 3
  - namn: Servera
    text: Peppra. Servera direkt med citronklyfta vid sidan.
tips:
  - >-
    Torsk går från perfekt till överstekt på 30 sekunder — lyft hellre för
    tidigt och lägg tillbaka. Eftervarme finns.
  - >-
    Mjöl eller panering skippas — onödigt på LCHF och torsken är god i sig
    själv.
  - >-
    En klick smör + citron + persilja rört i en skål = snabbt örtsmör som passar
    perfekt.
  - >-
    Frusen torsk: tina helt och torka extra noga, annars fäller fisken massor av
    vätska i pannan.
testlogg: []
---

Vardags-fisk när det ska gå snabbt. Torsk är magert och kräver fett från smör/sås — ensam blir den fadd. Saltning 5–10 min före stekning ger fastare köttstruktur. Stek inte för länge; flagar lätt vid 50–55°C innertemp.

## Modulkombinationer som funkar

- **Klassiker:** Torsk + broccolimos + hollandaise.
- **Vardag, snabbt:** Torsk + smörstekta haricots verts + Lohmanders bea.
- **Frisk:** Torsk + grön sallad + citronsmör + handpillade räkor över.
- **Lite mer:** Torsk + bakad rotsellerimos + senap-grädde-sås.

## Jessica-anpassning (delad fisk)

Båda äter samma direkt — ingen anpassning behövs. Torsk är hennes territorium också.

## Köpval

- **Färsk torsk** över MSC-märkt frusen om budget tillåter — bättre textur.
- **Frusen torskloin/-rygg** funkar utmärkt på vardag. Tina i kylen över natten eller snabbt i kallt vatten.
- **Inte lutfisk eller saltad torsk** — annan produkt.
