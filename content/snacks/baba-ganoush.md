---
namn: Baba ganoush (aubergine-paté)
status: skriven
portioner: 6
narings_per_portion:
  netto_kh: 3
  fett: 12
  protein: 3
  kcal: 140
tid:
  prep_min: 10
  tillagning_min: 40
  total_min: 50
forvantad_glukospaverkan: mycket-lag
taggar:
  - snacks
  - mellanostern
  - vegetariskt
  - kall
ingredienser:
  - mangd: 1
    enhet: st
    vara: stor aubergine
    notering: ca 400 g
  - mangd: 3
    enhet: msk
    vara: tahini
  - mangd: 2
    enhet: klyftor
    vara: vitlök
  - mangd: 2
    enhet: msk
    vara: pressad citron
  - mangd: 2
    enhet: msk
    vara: olivolja
  - mangd: 1
    enhet: tsk
    vara: spiskummin
  - mangd: 0.5
    enhet: tsk
    vara: smokad paprika
  - vara: 'salt, svartpeppar'
  - mangd: 1
    enhet: msk
    vara: 'färsk persilja, hackad'
    notering: topping
tillagning:
  - namn: Rosta aubergine
    text: >-
      Sätt ugn på 220°C. Stick i auberginen på flera ställen. Rosta hel på plåt
      40 min tills helt mjuk och kollapsad.
    timer_min: 40
  - namn: Skala
    text: Låt svalna 5 min. Skala av skinnet — ska vara enkelt. Skopa ut köttet.
  - namn: Mixa
    text: >-
      Lägg auberginkött, tahini, vitlök, citron, olivolja, spiskummin, paprika,
      salt och peppar i matberedare. Mixa till medium-grov pasta.
  - namn: Servera
    text: >-
      För över i skål. Ringla extra olivolja. Toppa med persilja. Servera med
      LCHF-pita eller grönsaksstavar.
tips:
  - Bra dippsås till tapas eller meze-bricka.
  - Håller 4 dagar i kyl.
testlogg: []
bild: "baba-ganoush.jpg"
bild_alt: "AI-bild: Baba ganoush (aubergine-paté)"
bild_ai: true
---


