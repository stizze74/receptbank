---
namn: Borsjt LCHF (rysk rödbetssoppa)
status: skriven
portioner: 4
narings_per_portion:
  netto_kh: 6
  fett: 18
  protein: 4
  kcal: 220
tid:
  prep_min: 15
  tillagning_min: 35
  total_min: 50
forvantad_glukospaverkan: lag
taggar:
  - soppa
  - rysk
  - vegetariskt
  - vinter
ingredienser:
  - mangd: 200
    enhet: g
    vara: 'rödbeta, skalad'
  - mangd: 1
    enhet: st
    vara: gö lök
  - mangd: 2
    enhet: klyftor
    vara: vitlök
  - mangd: 5
    enhet: dl
    vara: nötbuljong
  - mangd: 1
    enhet: dl
    vara: crème fraiche
  - mangd: 1
    enhet: msk
    vara: rödvinsvinäger
  - mangd: 30
    enhet: g
    vara: smör
  - mangd: 1
    enhet: st
    vara: lagerblad
  - vara: 'salt, svartpeppar'
  - mangd: 2
    enhet: msk
    vara: färsk dill
    notering: topping
tillagning:
  - namn: Fräs
    text: Smält smör i kastrull. Fräs finhackad lök och vitlök 5 min.
    timer_min: 5
  - namn: Tillsätt rödbeta
    text: 'Tärna rödbetan. Tillsätt i kastrullen, fräs 3 min.'
  - namn: Sjud
    text: 'Häll i buljong, vinäger och lagerblad. Sjud 30 min tills rödbetan är mör.'
    timer_min: 30
  - namn: Mixa
    text: Plocka bort lagerblad. Mixa till len soppa. Smaka av.
  - namn: Servera
    text: Fördela i skålar. Klicka på crème fraiche och strö dill.
tips:
  - >-
    Rödbeta innehaller mer kh än många andra grönsaker (~6g/100g netto), så
    portion hålls medel. Verifiera mot CGM.
  - >-
    Klassisk borsjt har vanligen socker — LCHF-version har vinäger som
    syra-balans.
testlogg: []
bild: "borsjt-lchf.jpg"
bild_alt: "AI-bild: Borsjt LCHF (rysk rödbetssoppa)"
bild_ai: true
---


